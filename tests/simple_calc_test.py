...
def test_simple_calc_add_cli():
    runner = CliRunner()
    result = runner.invoke(sc.int_group, ["add", "1", "2", "3"])
...
def test_simple_calc_mult():
    runner = CliRunner()
    result = runner.invoke(sc.int_group, ["mult", "1", "2", "3", "4"])
    assert result.exit_code == 0
...
def test_simple_calc_mult_ten_numbers():
    runner = CliRunner()
    result = runner.invoke(sc.int_group, ["mult"] + [str(i) for i in range(1, 11)])
    assert result.exit_code == 0
